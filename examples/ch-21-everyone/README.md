# Ch. 21 — Personal Workflows Exercise

A small fictional bank statement for one month.

## Exercise

> Read `bank-statement.txt`, summarize the month, group expenses into categories, and flag anything that looks unusual or like a forgotten subscription.

The agent should catch the `ABC LIMITED` charge for HKD 0.99 — a classic "free trial converted to a real subscription" pattern with a deliberately vague merchant name. A good agent flags it without being told to look for it.
