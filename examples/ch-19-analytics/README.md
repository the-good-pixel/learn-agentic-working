...
- **`Transactions`** (~285 rows) — Sep, Oct, Nov 2025 across three currencies (HKD primary, some USD subscriptions, JPY for a Tokyo trip). Columns: `date, description, memo, category_raw, amount, currency, balance_after`. `balance_after` is a running balance computed with formulas that convert non-HKD rows via the FX Rates sheet.
- **`FX Rates`** — USD→HKD and JPY→HKD rates for the period, with a "rates are approximate; use these for analysis" note.
...
- **Edge case: `AMAZON.CO.JP charged in HKD`** — Amazon Japan, but billed in HKD. Tempting to mis-bucket as Travel; it's Shopping.
- **No true duplicate transactions.** Repeated `STARBUCKS-COFFEE-CENTRAL` at HKD 58 is a recurring pattern, not a duplicate. Two `Rent transfer - landlord WONG` entries on different months are normal monthly rent, not a duplicate. Exercise 3 is partly a test of restraint.
- **Salary deposits** ~monthly — these are income, not spend. Exclude from spend totals.

## Exercise 1 — Categorize and rank

> Read `transactions-nov.xlsx`, decode the `memo` column into clean categories, and tell me my top 5 categories of **spend** in HKD-equivalent. Exclude income (salary deposits) from the totals. State the FX rates you used.
