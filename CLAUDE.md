...
- **`i18n-reviewer`** (project-level, `/Users/darrenchiu/Development/tl-mono/.claude/skills/i18n-reviewer/SKILL.md`) — audits the frontend for missing translation keys *and* enforces geopolitical naming (e.g. "Country/Region" not "Country" when Taiwan/HK are in the list, for mainland-investor sensitivity). **Skill encoding policy, not just code.** Best for: Ch. 22, Ch. 27.
