...
**"Why doesn't this Threads post show a link preview?"** You screenshot the broken preview, the agent looks at it alongside the URL, fetches the page, and walks you through which Open Graph tag is missing. (You'll meet this scenario again in Ch. 23 from the *browser-driving* side.)
...
- **Images cost tokens too.** A high-resolution screenshot can easily run a few thousand tokens. Not free, not expensive. Don't worry about it for one-off use; do think about it if you're building a workflow that processes hundreds of images per run (more on cost in Ch. 24).
