...
Only when none of those exist do you drive the browser ad-hoc. And when you do drive it ad-hoc twice, that's the signal to capture the third run as a skill (Ch. 17).
...
- **Drift from "I'll just watch it once" to "I'll just trust it."** Headed mode lulls you into watching less over time. For high-stakes actions (anything financial, anything irreversible — see Ch. 25), keep a gate even when the agent has done it ten times. For reversible things, let it cook.
...
- When you find yourself driving the same flow twice, capture it as a skill. The third run is one command (see Ch. 17, Ch. 18).
