...
A Git feature that lets you have multiple working copies of the same repository checked out at the same time, each on a different branch, in separate folders. Engineers use worktrees to run several agents in parallel — one per ticket — without them stepping on each other's branches, build caches, or `node_modules` folders. The canonical pattern is **"worktree-per-ticket"**: one agent in one folder works on one Linear ticket from start to PR. **This is engineer-only**; non-engineers can safely skip it. If you're not a developer and you saw the term in Ch. 15, the relevant thing to know is that engineers have a clean way to run agents in parallel; the mechanics aren't yours to worry about.
*→ Ch. 15*
