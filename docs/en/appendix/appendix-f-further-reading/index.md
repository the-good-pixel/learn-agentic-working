...
- **Anthropic's "Introducing computer use" post** (October 2024) and the follow-on technical writeups. Search `anthropic.com` for "computer use". Read these before Ch. 26.
...
- **Anthropic's posts on evals.** Search `anthropic.com/engineering` for "evals" and "evaluation". They've published the clearest writing on lightweight evals for non-researchers — directly relevant to Ch. 27.
