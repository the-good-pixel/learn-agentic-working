...
If you have multiple accounts in different markets (one US broker, one Vancouver broker, one Japan account), the merging-across-accounts step is exactly the kind of plumbing that nobody else will do for you. The agent will.
...
1. **Flagship — decode a bank statement.** Run against [`examples/ch-21-everyone/bank-statement.pdf`](/examples/ch-21-everyone/bank-statement.pdf):
   ...
   **You'll know it worked when** the agent flags the recurring `ABC LIMITED` USD 0.99 charge — a planted "free trial converted to a real subscription" pattern with a deliberately vague merchant name. There's a second planted oddity: a **duplicate rent payment** to landlord JOHNSON on Nov 4 and Nov 28 (the second one labeled slightly differently so it looks plausibly distinct). The prompt above primes the agent to look for *something*. For a harder test of whether *your particular agent* surfaces oddities unprompted, also try the neutral version: *"Read the file and summarize my month."* — and see whether either the `ABC LIMITED` charge or the duplicate rent shows up in the summary at all. The gap between the two answers tells you how much initiative your agent takes on its own.
