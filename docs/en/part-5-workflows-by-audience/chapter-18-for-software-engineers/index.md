...
It writes you a map. Then you ask follow-ups: *"how does auth work end-to-end?"*, *"what's `WorkflowDispatcher` actually for?"*, *"why are there two different cron systems?"* (and yes — sometimes there really are two; a good codebase has a skill that documents the choice, see Ch. 17).
...
Each repeated correction is a signal. Write it down as a skill (Ch. 16, Ch. 17) — or rather, finish the session and say *"that was useful; write a skill from what we just did."* The agent reads back over the conversation, picks out the corrections and gotchas, and drafts the skill for you. You read it, save it. Next time, the correction isn't needed.
