...
There's also a *policy* shape of product work that's easy to miss. If your product ships in regions with sensitive geopolitics, you've probably been bitten once by a wording choice — for example, listing Taiwan and Hong Kong under a "Country" dropdown rather than a "Country/Region" one, which can lose you a market overnight. That kind of rule is the perfect thing to write down as a reusable workflow the agent runs on every release. The agent learns it once (you tell it, in plain English) and from then on it flags any release that violates the rule. The workflow encodes *policy*, not just code.
