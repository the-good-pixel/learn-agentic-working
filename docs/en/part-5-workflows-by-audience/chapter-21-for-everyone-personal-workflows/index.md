...
1. **Flagship — decode a bank statement.** Run against `examples/ch-21-everyone/bank-statement.txt`:
   ...
   **You'll know it worked when** the agent flags the `ABC LIMITED` HKD 0.99 charge without being told to look for it — a planted "free trial converted to a real subscription" pattern with a deliberately vague merchant name.
